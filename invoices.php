...
<div class="list-container">
    <h2>All Invoices</h2>
    <?php
    $stmt = $pdo->query("SELECT id, number, customer_name, date, total, status FROM documents WHERE type='invoice' ORDER BY date DESC, id DESC");
    $rows = $stmt->fetchAll();
    ?>
    <table class="list">
        <thead>
        ...
        <?php foreach ($rows as $r): ?>
            <tr>
                ...
                <td class="actions">
                    <a href="/billing-web/views/invoice.php?id=<?= (int)$r['id'] ?>">View</a>
                    <a href="/billing-web/views/invoice.php?number=<?= urlencode($r['number']) ?>">By Number</a>
                </td>
            </tr>
        <?php endforeach; ?>
        <?php if (!$rows): ?>
